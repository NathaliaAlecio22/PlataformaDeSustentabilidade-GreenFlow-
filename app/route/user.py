...
def put_consumption(request):
    ...
    # Campos obrigatórios para a edição de consumo
    ...
    try:
        # Tenta atualizar o relatório
        update_report(relatorio)

        # Pega o id do relatório atualizado
        comando_rel = f"""
...
def profile():
    ...
    nr_norm = Statistics.normalize('non_recyclabe_residue', rep.get('ResiduosR', 0))
...
# "/relatorio/1?user_id=1" exemplo de link com argumentos
