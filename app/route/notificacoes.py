...
def notificar_variacao_isp(user_id):
    # Pega o último relatório do usuário
    comando_rel = f"""
        ...
        LIMIT 1
    """
    relatorio = executar_comando("GET", comando_rel)
    if not relatorio:
        return
    
    rel_id = relatorio[0][0]
    
    # Pega os dois últimos consumos normalizados para esse relatório
    comando = f"""
        SELECT ID, ISP 
    ...
    id_consumo_normalizado_atual, isp_hoje = resultados[0]
    ...
    comando_insert = f"""
        ...
        VALUES ('{titulo}', '{conteudo}', {id_consumo_normalizado_atual})
    """
...
def notificar_consumo_bruto(user_id):
    # Pega o último relatório do usuário
    comando_rel = f"""
        SELECT ID FROM relatorios
        WHERE fk_id_usuario = {user_id}
    ...
    # Pega os dois últimos consumos brutos para esse relatório
    comando_consumos = f"""
        SELECT 
